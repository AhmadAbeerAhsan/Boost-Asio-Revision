    ...
    void onConnect_ReadRequest()
    {
        asio::async_read_until(
            *m_sock.get(),
            ...
            {
                me->onReadRequest_ProcessRequest(ec, bytes_transferred);
            });
    }

    void onReadRequest_ProcessRequest(const boost::system::error_code &ec, std::size_t bytes_transferred)
    {
        if (ec)
        {
        ...
        asio::async_write(
            *m_sock.get(),
            ...
            [me = shared_from_this()](const boost::system::error_code &ec, std::size_t bytes_transferred)
            {
                me->onResponseSent(ec, bytes_transferred);
            });
    }
...
private:
    void InitAccept()
    {
        std::shared_ptr<asio::ip::tcp::socket> sock{std::make_shared<asio::ip::tcp::socket>(m_ioc)};
    ...
    void onAccept(const boost::system::error_code &ec, std::shared_ptr<asio::ip::tcp::socket> sock)
    {
        if (!ec)
        {
            std::shared_ptr<Service> handler = std::make_shared<Service>(sock);
...
{
public:
    Server() : m_work(asio::make_work_guard(m_ioc))
    {
    }
