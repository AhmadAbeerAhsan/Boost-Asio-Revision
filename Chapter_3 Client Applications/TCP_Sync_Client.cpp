...
public:
  SyncTCPClient(const std::string &raw_ip_address,
                unsigned short port_num) : m_ep(asio::ip::make_address(raw_ip_address), port_num),
                                           m_sock(m_ioc)
  {
  ...
  void close()
  {
    ...
    // shuts the connection down and closes the socket, which leads to the operating system's socket and other resources associated with it to be deallocated.
    m_sock.close();
  }
