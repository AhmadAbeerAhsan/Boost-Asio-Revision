...
public:
    ...
    unsigned int request_id)
  {
    ...
    session->m_sock.async_connect(
      ...
      {
        onConnect_SendRequest(ec, session);
      }
    );
  };
...
private:
  ...
  void onConnect_SendRequest(const boost::system::error_code& ec, std::shared_ptr<Session> session)
  {
    if (ec)
    {
      session->m_ec = ec;
    ...
    (
      ...
      {
        onSendRequest_ReadResponse(ec, bytes_transferred, session);
      }
    );
  }

  void onSendRequest_ReadResponse(const boost::system::error_code& ec, std::size_t bytes_transferred, std::shared_ptr<Session> session)
  {
    if (ec)
    {
      session->m_ec = ec;
    ...
    asio::async_read_until(
      ...
      [this, session](const boost::system::error_code& ec, std::size_t bytes_transferred) 
      {
        onReadResponse_ProcessResponse(ec, bytes_transferred, session);
      }
    );
  }

  void onReadResponse_ProcessResponse(const boost::system::error_code& ec, std::size_t bytes_transferred, std::shared_ptr<Session> session)
  {
    if (ec)
    {
      session->m_ec = ec;
